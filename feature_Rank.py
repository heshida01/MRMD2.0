...
import  feature_selection.lasso2
...
class PRMapReduce:
    ...
    def page_rank(self):
        ...
        return self.graph
def init(mrmd,ANOVA,mrmr,mic,lasso,lasso2):
...
def feature_rank(file,logger,mrmr_length):
    ...
    mrmr_data = feature_selection.mrmr.run(file,logger,mrmr_length)
    lasso_data = feature_selection.lasso.run(file,logger)
    lasso2_data = feature_selection.lasso2.run(file)
    edge=init(mrmd_data,ANOVA_data,mrmr_data,mic_data,lasso_data,lasso2_data)
